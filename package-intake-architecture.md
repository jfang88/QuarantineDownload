...
| 1.10 | 2026-08-02 | Security Architecture | Corrected the Overview's unverified "Notepad++ 8.6.4" claim to the real, verified Notepad++ "Chrysalis" update-infrastructure compromise (CVE-2025-15556); added XZ Utils, SolarWinds, and CCleaner as further delayed-discovery precedent; replaced Stage 11b's flat nightly/weekly recheck cadence with an age-based tapering schedule (daily for 0–30 days since promotion, weekly through 180 days, monthly after) and updated the rate-limit budgeting logic to allocate across both risk tier and age cadence |
| 1.11 | 2026-08-02 | Security Architecture | Added fast-detection precedent (`ua-parser-js`, ~4-hour exposure; PyTorch `torchtriton`, ~5-day exposure) to the Overview to justify the 0–30-day daily recheck tier; added real compromised-model-weight precedent for Path C (JFrog's 100+ malicious Hugging Face models; the PoisonGPT proof-of-concept), with all four claims added to References as verified this session |
| 1.12 | 2026-08-02 | Security Architecture | Added recent high-profile precedent to the Overview: the Sept 2025 npm `chalk`/`debug` compromise, the Shai-Hulud self-propagating npm worm (Sept–Nov 2025, CISA alert), the March 2026 `axios`/`LiteLLM` compromises (the latter in the same wave as the Trivy compromise already cited in this document), the May 2026 "Megalodon" GitHub campaign (CISA alert), and two older canonical incidents (Polyfill.io 2024, 3CX 2023); all seven added to References with primary-source citations verified this session |
...
**A fourth concern — post-approval supply chain compromise — requires controls that none of the three paths above fully addresses.** SBOM-based tools such as Dependency-Track detect known CVEs in component versions, but they cannot detect a trojanised binary where the version string is correct and no CVE has been published. A compromised Trivy binary is one example. A legitimate application whose *update-delivery channel* is hijacked — rather than its source or tagged releases being altered — is another, and is not a hypothetical: the Notepad++ "Chrysalis" backdoor (CVE-2025-15556) was a real compromise of Notepad++'s own update-distribution infrastructure, active from June to December 2025 and not publicly disclosed until February 2026, that redirected the application's auto-updater to a malicious installer without altering any officially tagged source release. Neither Dependency-Track's CVE matching nor a component-name-and-version check would have caught it, because nothing about the recorded component identity ever changed — only the bytes served outside this pipeline's control. The same blind spot applies to any binary that was clean at intake and later identified as part of a supply chain attack campaign, and to a model checkpoint that was clean when pinned but whose mutable upstream reference (a branch or tag, not the pinned commit itself) has since drifted to point at different, unreviewed content.

This delayed-discovery pattern is common, not unusual. The XZ Utils backdoor (CVE-2024-3094) was merged into release tarballs roughly a month before a Microsoft engineer noticed anomalous SSH latency and traced it back — a near-miss caught before it reached major distributions' stable branches. The 2020 SolarWinds Orion compromise sat undetected for roughly 14 months between the backdoor's insertion and its public discovery. The 2017 CCleaner compromise shipped a backdoored, legitimately signed build to an estimated 2.27 million users for about a month before detection. In every case, valid signatures and correct version metadata were exactly what let the compromise pass initial checks undetected. Stage 11b in this architecture addresses this gap through retroactive hash rechecks, updated YARA scans, and binary authentication controls applied periodically — and on an age-based tapering schedule, weighted toward the window where these examples show detection is most likely — against the entire approved artifact inventory.

Detection time is not always measured in months, and the fast end of the spectrum is what the 0–30-day daily recheck tier is sized around. The `ua-parser-js` npm package (roughly 8 million weekly downloads) was compromised for about four hours on 22 October 2021 — three versions carried a cryptominer and credential-stealing payload before community reports on GitHub and a public Slack channel triggered removal; CISA issued an alert the same day. PyTorch's own `torchtriton` nightly dependency was hijacked via dependency confusion on PyPI between 25 and 30 December 2022; the PyTorch team disclosed the compromise within the same window, roughly five days after the malicious package first went live. Both fall well inside the daily tier — most fast detections land within days to weeks of exposure, which is why that tier exists as a distinct, denser cadence rather than folding into the general weekly/monthly schedule.

AI/ML model artifacts carry the same class of risk in a different form, which is what Path C's provenance and sandboxing controls exist to address. In February 2024, JFrog's security research team identified more than 100 malicious models publicly hosted on the Hugging Face Hub — about 95% built on PyTorch — that used a pickle `__reduce__` payload to execute arbitrary code, including reverse shells, the moment a victim called `torch.load()` on the file. Nothing about the file's declared name or version flagged it; the malicious behaviour was embedded inside the serialization format itself, the same class of risk this document's safetensors-over-pickle guidance addresses. Separately, researchers at Mithril Security published PoisonGPT in 2023 as a deliberate proof-of-concept, not a real attack campaign: they used model-editing (ROME) to surgically alter a single fact inside GPT-J-6B — a technique specifically intended to leave the rest of the model's behaviour, and therefore its performance on standard benchmarks, largely unaffected — then uploaded the result to a Hugging Face namespace one character off from a legitimate organisation's name. It was downloaded more than 40 times before Hugging Face removed it for a terms-of-service violation, not because any automated scan caught the tampering. Both examples are the direct justification for Path C's position that a passing benchmark result or a plausible-looking namespace is not evidence of safety on its own — see Stage 4c provenance verification and Stage 6's isolated model load-test sandbox.

**None of this is a slow-moving or historical problem — the pace and scale of these incidents increased sharply in the twelve months before this document's current revision.** On 8 September 2025, a phishing attack against a single maintainer compromised 18 widely used npm packages — including `chalk` and `debug`, together downloaded over 2.6 billion times per week — with malicious versions live in the registry for roughly 16 minutes before deployment and flagged by the community within about two hours; the payload rewrote cryptocurrency payment destinations in-browser. That same month, a separate self-propagating worm dubbed "Shai-Hulud" began using each compromised npm maintainer account to infect the packages that account maintained, spreading further with every new infection; by the time CISA issued a public alert, it had compromised more than 700 npm packages and generated over 27,000 malicious GitHub repositories, with a second wave ("Shai-Hulud V2") detected that November. In March 2026, a North Korea-attributed group took over the `axios` npm package's lead maintainer account by changing its registered email, bypassing the project's CI/CD protections entirely and shipping a cross-platform remote-access-trojan dropper; the same month, a separate campaign (tracked as "TeamPCP") compromised the PyPI package `LiteLLM` — a library for interfacing with LLM APIs, directly inside this document's own problem space — to harvest AWS, GCP, and Azure credentials, in the same wave that also hit Trivy, already cited above as this document's own cautionary example for the pipeline-tooling-supply-chain risk. In May 2026, a campaign codenamed "Megalodon" pushed over 5,700 malicious commits to more than 5,500 GitHub repositories in a single six-hour window, using forged bot-like commit identities to inject credential-harvesting CI/CD workflows — CISA issued a second alert for this one too.

Two further, older incidents are worth naming because they are widely cited as the canonical examples of this problem, independent of the most recent wave. The Polyfill.io compromise (disclosed June 2024) is a close parallel to the Notepad++/Chrysalis case above: a company acquired the domain and GitHub account behind a CDN-hosted JavaScript library embedded on an estimated 100,000 to 380,000+ websites — including JSTOR, Intuit, and the World Economic Forum — and began serving malware through it. The code that had originally been reviewed was never altered; only the infrastructure serving it changed hands. The 2023 3CX DesktopApp compromise, attributed to the North Korea-linked Lazarus Group, was a cascading attack: attackers first compromised an unrelated trading-software vendor (Trading Technologies' X_Trader), used that foothold to compromise a single 3CX employee's machine, and used that access to backdoor legitimately signed 3CX installers distributed to 3CX's own customers — one vendor's supply chain compromise becoming the entry point into a second vendor's supply chain, the exact multi-hop risk this architecture's own pipeline-tooling-supply-chain section is designed to contain.
...
This stage addresses the specific threat of post-approval supply chain compromise. It runs as a scheduled job and operates against the complete inventory of approved artifacts stored in Nexus, regardless of how long ago they were approved.

**Recheck frequency is age-based, not uniform.** Newly promoted artifacts are checked most often, tapering to a lower frequency as they accumulate a longer clean recheck history:

| Artifact age since promotion (or since last cadence reset) | Recheck frequency |
|---|---|
| 0–30 days | Daily |
| 31–180 days | Weekly |
| 180+ days | Monthly |

The 0–30-day window is deliberately the densest: AV-engine and threat-intel coverage for a given hash typically grows fastest in the weeks immediately after a compromise becomes known, and every historical example cited in the Overview above (Notepad++/Chrysalis, XZ Utils, SolarWinds, CCleaner) was caught within roughly a month to just over a year, not years later — the early window is where a recheck is most likely to convert into an actual catch. Tier 1 artifacts (developer toolchain, privileged binaries) stay on the daily schedule regardless of age, consistent with the risk-tier model used elsewhere in this stage.

A cadence reset — an artifact re-enters the 0–30-day daily tier — is triggered by: a new version promoted under the same component name, a confirmed false-positive disposition being overturned, or a manual reset by a pipeline administrator in response to a specific threat-intel prompt (for example, a new IOC report naming the artifact's publisher).

This tapering reduces, but does not eliminate, the budget-growth problem described below: it caps the steady-state size of the "always daily" cohort at roughly (30 days × average daily promotion rate) rather than letting it grow with total inventory size, but the 31+ day cohorts still grow with total inventory size, just at a lower per-artifact query rate. It also does not help against a channel-level compromise like Notepad++/Chrysalis on its own — a recheck re-queries threat intel for a hash *already sitting in the store*; it cannot detect that a fresh download from the vendor today would now be malicious. That risk is addressed by Stage 4's independent hash/signature verification at intake, not by Stage 11b.
...
**Rate-limit and coverage budgeting.** External lookup services are rate-limited (VirusTotal's free tier is 500 lookups/day; MalwareBazaar and NSRL have their own practical limits at bulk-query volume). The recheck job allocates its daily query budget along two dimensions together: risk tier (Tier 1 and developer-toolchain artifacts get priority queue placement) and the age-based cadence above (the 0–30-day cohort runs daily regardless of risk tier, since that is where the delayed-discovery pattern described in the Overview is most likely to produce a hit; older cohorts compete for remaining budget on their weekly/monthly schedule). When both dimensions together still exceed the daily budget, Tier 2/3 artifacts in the 31+ day cadence are queued and rotated to fit the remaining budget, so that coverage degrades predictably (a defined, reported staleness window per tier) rather than silently (some artifacts never getting rechecked without anyone noticing). The coverage rotation schedule, the age-cadence boundaries above, and the paid-tier upgrade threshold are implementation decisions — see [ADR-0004](adr/0004-virustotal-public-api-usage-and-licensing.md).
...
| The Notepad++ "Chrysalis" backdoor was a real 2025–2026 supply-chain compromise of the update-*delivery infrastructure* (not a backdoored tagged source release), active June–December 2025, publicly disclosed February 2026, and assigned CVE-2025-15556 | [Tenable — Frequently Asked Questions About the Notepad++ Supply Chain Compromise](https://www.tenable.com/blog/frequently-asked-questions-about-notepad-supply-chain-compromise), [Picus Security — Notepad++ Supply Chain Attack: Chrysalis Backdoor](https://www.picussecurity.com/resource/blog/notepad-supply-chain-attack-chrysalis-backdoor) | Confirmed the compromise mechanism (hosting-provider-level update-infrastructure hijack, not a tainted release), the timeline, and the CVE assignment. This replaced an earlier version of this document's Overview, which asserted a specific "Notepad++ 8.6.4" version number that neither source confirms — that number was not independently verified and has been removed rather than left as an unsupported specific claim |
| The `ua-parser-js` npm package was compromised for approximately four hours on 22 October 2021 (versions 0.7.29, 0.8.0, 1.0.0 carrying a cryptominer/credential-stealer) before community reports triggered removal | [CISA — Malware Discovered in Popular NPM Package, ua-parser-js](https://www.cisa.gov/news-events/alerts/2021/10/22/malware-discovered-popular-npm-package-ua-parser-js), [Rapid7 — NPM Library (ua-parser-js) Hijacked](https://www.rapid7.com/blog/post/2021/10/25/npm-library-ua-parser-js-hijacked-what-you-need-to-know/) | Confirmed the ~4-hour exposure window and the compromised version numbers |
| PyTorch's `torchtriton` nightly dependency was hijacked via PyPI dependency confusion between 25–30 December 2022; PyTorch disclosed the compromise on 30 December 2022, within the same window | [PyTorch — Compromised PyTorch-nightly dependency chain between December 25th and December 30th, 2022](https://pytorch.org/blog/compromised-nightly-dependency/) | Confirmed via PyTorch's own incident disclosure — a primary source, not a third-party summary |
| JFrog identified 100+ malicious models on the Hugging Face Hub (published February 2024) using pickle `__reduce__` payloads to execute code, including reverse shells, on `torch.load()`; ~95% were PyTorch-format | [JFrog — Data Scientists Targeted by Malicious Hugging Face ML Models with Silent Backdoor](https://jfrog.com/blog/data-scientists-targeted-by-malicious-hugging-face-ml-models-with-silent-backdoor/) | Confirmed via JFrog's own research disclosure — a primary source |
| PoisonGPT (2023) was a deliberate proof-of-concept by Mithril Security, not a real attack campaign: GPT-J-6B was edited with ROME to alter one fact, uploaded to a typosquatted Hugging Face namespace, and downloaded 40+ times before removal for a ToS violation — not because a scan caught the tampering | [Mithril Security — PoisonGPT: How We Hid a Lobotomized LLM on Hugging Face to Spread Fake News](https://blog.mithrilsecurity.io/poisongpt-how-we-hid-a-lobotomized-llm-on-hugging-face-to-spread-fake-news/) | Confirmed via the researchers' own primary write-up, including the download count and removal reason |
| On 8 September 2025, a phishing attack against a `chalk`/`debug` maintainer compromised 18 npm packages (2.6B+ weekly downloads); malicious code was live ~16 minutes before deployment and community-flagged within ~2 hours | [Semgrep — chalk, debug and color on npm compromised in new supply chain attack](https://semgrep.dev/blog/2025/chalk-debug-and-color-on-npm-compromised-in-new-supply-chain-attack/), [Wiz — Widespread npm Supply Chain Attack](https://www.wiz.io/blog/widespread-npm-supply-chain-attack-breaking-down-impact-scope-across-debug-chalk) | Confirmed the compromise date, package list, download volume, and the ~16-minute-to-2-hour timeline |
| The "Shai-Hulud" self-propagating npm worm (September 2025, V2 in November 2025) compromised 700+ npm packages and generated 27,000+ malicious GitHub repositories by using each compromised maintainer account to infect further packages | [CISA — Widespread Supply Chain Compromise Impacting npm Ecosystem](https://www.cisa.gov/news-events/alerts/2025/09/23/widespread-supply-chain-compromise-impacting-npm-ecosystem) | Confirmed via a US government advisory — the highest-authority source used in this document's References |
| In March 2026, a North Korea-attributed group compromised the `axios` npm package's lead maintainer account (email change, bypassing CI/CD) to ship a cross-platform RAT dropper; separately, the "TeamPCP" campaign compromised the PyPI package `LiteLLM` (credential-harvesting payload) in the same wave that hit Trivy | [Zscaler ThreatLabz — Axios NPM & LiteLLM PyPI Supply Chain Attacks (March 2026)](https://www.zscaler.com/blogs/security-research/supply-chain-attacks-surge-march-2026) | Fetched and confirmed directly this session: both incidents are real, attributed to two separate threat actors, and the LiteLLM/Trivy/KICS/Telnyx compromises are the same March 2026 wave as the Trivy compromise already cited above |
| In May 2026, the "Megalodon" campaign pushed 5,700+ malicious commits to 5,500+ GitHub repositories in a six-hour window using forged bot commit identities to inject credential-harvesting CI/CD workflows | [CISA — Supply Chain Compromises Impact Nx Console and GitHub Repositories](https://www.cisa.gov/news-events/alerts/2026/05/28/supply-chain-compromises-impact-nx-console-and-github-repositories), [SecurityWeek — Over 5,500 GitHub Repositories Infected in 'Megalodon' Supply Chain Attack](https://www.securityweek.com/over-5500-github-repositories-infected-in-megalodon-supply-chain-attack/) | Confirmed via a US government advisory plus independent security-press reporting on the commit count, repository count, and six-hour window |
| The Polyfill.io compromise (disclosed June 2024) was a domain/account-ownership change, not a code compromise: an acquirer took over the CDN domain and GitHub account behind a JavaScript library embedded on 100,000–380,000+ sites and began serving malware through it | [Sonatype — Polyfill.io Supply Chain Attack Explained](https://www.sonatype.com/blog/polyfill.io-supply-chain-attack-hits-100000-websites-all-you-need-to-know), [The Hacker News — Polyfill[.]io Attack Impacts Over 380,000 Hosts](https://thehackernews.com/2024/07/polyfillio-attack-impacts-over-380000.html) | Confirmed the ownership-change mechanism, site-count estimates, and named affected sites (JSTOR, Intuit, World Economic Forum) |
| The 2023 3CX DesktopApp compromise was a cascading double supply-chain attack: Lazarus Group compromised trading-software vendor Trading Technologies' X_Trader first, then used that foothold to backdoor legitimately signed 3CX installers | [Volexity — 3CX Supply Chain Compromise Leads to ICONIC Incident](https://www.volexity.com/blog/2023/03/30/3cx-supply-chain-compromise-leads-to-iconic-incident/), [Mandiant/Google Cloud — 3CX Software Supply Chain Compromise](https://cloud.google.com/blog/topics/threat-intelligence/3cx-software-supply-chain-compromise) | Confirmed the two-vendor cascading mechanism and Lazarus Group attribution via primary incident-response vendor writeups |
...
- The XZ Utils backdoor (CVE-2024-3094), the 2020 SolarWinds Orion compromise, and the 2017 CCleaner compromise, cited in the Overview as further examples of delayed-discovery supply chain compromises with valid signatures/version metadata — well-established, widely reported incidents, but not re-fetched against a primary source in this pass (the XZ Utils incident was already referenced elsewhere in this document, in Stage 11b Step 4, before this reference-verification pass existed)
